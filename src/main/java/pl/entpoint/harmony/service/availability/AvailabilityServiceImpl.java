...
public class AvailabilityServiceImpl implements AvailabilityService {
    ...
    @Override
    @Transactional
    public void create(LocalDate date, List<DayOffPojo> dayOffs) {
        Optional<Availability> optional = Optional.ofNullable(availabilityRepository
                    .findByAvailabilityDate(date));

        if (optional.isPresent()){
            throw new IllegalArgumentException("Dyspozycyjność o podanej dacie istenie już w bazie");
        } else {
            Availability availability = new Availability();
            availability.setAvailabilityDate(date);
            availability.setActive(true);
            if(dayOffs != null) {
                for(DayOffPojo dayOffPojo : dayOffs) {
                    dayOffService.create(new DayOff(dayOffPojo));
                }
            }
